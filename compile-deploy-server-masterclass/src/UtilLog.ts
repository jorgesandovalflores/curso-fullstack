    ...
    public static logMiddleware(): any {
        return async (ctx: Context, next: () => Promise<any>): Promise<void> => {
            ...
            if (ctx.status >= 500) {
                ...
                UtilLog.logError(`[${requestId}] # user-agent = ${ctx.get('user-agent')}`)
                UtilLog.logError(`[${requestId}] # x-timezone = ${ctx.get('x-timezone')}`)
                ...
                UtilLog.logWarn(`[${requestId}] # user-agent = ${ctx.get('user-agent')}`)
                UtilLog.logWarn(`[${requestId}] # x-timezone = ${ctx.get('x-timezone')}`)
                UtilLog.logWarn(`[${requestId}] # response = ${JSON.stringify(ctx.response.body)}`)
            } else {
                UtilLog.logSuccess(`[${requestId}] # ${message}`)
                if (ctx.method != 'GET') {
                    UtilLog.logInfo(`[${requestId}] # request = ${JSON.stringify(ctx.request.body)}`)
                }
                UtilLog.logInfo(`[${requestId}] # user-agent = ${ctx.get('user-agent')}`)
                UtilLog.logInfo(`[${requestId}] # x-timezone = ${ctx.get('x-timezone')}`)
